  ...
  <div class="container py-3">
    ...
    <div class="table-responsive">
      <table class="table table-bordered" id="tabel-siswa">
        ...
        <tbody id="data-siswa-container"></tbody>
      </table>
    </div>
...
<script>
  const dataSiswa = <?php echo json_encode($dataSiswa ?? []); ?>;
  ...
  const tbody = document.getElementById('data-siswa-container');
  const paginationContainer = document.getElementById('pagination-container');

  function tampilkanDataSiswa(halaman) {
    const indexAwal = (halaman - 1) * jumlahDataPerHalaman;
    const dataHalamanIni = dataSiswa.slice(indexAwal, indexAwal + jumlahDataPerHalaman);
    let html = '';

    dataHalamanIni.forEach((siswa, index) => {
      html += `
        <tr>
          <td>${indexAwal + index + 1}</td>
          <td>${siswa.Nama}</td>
          <td>
            <a href="${baseUrl}overview/detailSiswa?id=${siswa.Id}" class="btn btn-sm btn-primary">Lihat Detail</a>
          </td>
        </tr>
      `;
    });
  ...
  document.querySelectorAll('.page-link').forEach(link => {
    link.addEventListener('click', function (e) {
      ...
      if (!isNaN(targetHalaman)) {
        ...
        tampilkanDataSiswa(halamanAktif);
        buatPagination();
